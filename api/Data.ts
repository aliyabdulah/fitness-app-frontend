...
export const bookTrainer = async (trainerId: string, sessionData: {
  date: string;
  time: string;
  duration: number;
  sessionType: string;
}) => {
  const { data } = await api.post(`/api/trainers/${trainerId}/book`, sessionData);
  return data;
};
...
export const reserveEquipment = async (equipmentId: string, reservationData: {
  date: string;
  startTime: string;
  duration: number;
}) => {
  const { data } = await api.post(`/api/equipment/${equipmentId}/reserve`, reservationData);
  return data;
};
...
export const challengeFriend = async (friendId: string, challengeData: {
  type: string;
  target: number;
  duration: number;
  description: string;
